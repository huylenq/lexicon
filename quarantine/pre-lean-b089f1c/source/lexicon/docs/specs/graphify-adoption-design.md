...
> **Status: refuted (2026-07-15) — scoped to the curation-feed direction.** This spec is kept as the record of a measured negative result, not as an active design. What is refuted is graphify as an **invisible curation feed**: Decisions 3 and 4 (bootstrap seeding, drift feed) failed their gates (see the inline refutation notes and the P1/P2 results block); the P1 implementation was reverted; Decision 1 — keep the viewer's code-intel stack, change nothing — is the only decision that survives. Lexicon's curation moves use no graphify. Do not promote this spec; do not re-explore the seeding/drift-feed direction without new evidence that invalidates the numbers below.
>
> **Repivot (2026-07-15).** The direction this run never tested — graphify's graph as a *visible viewer surface* (territory display, symbol-relationship browsing) — is picked up by `viewer/lexicon/specs/graphify-lens-design.md`. That spec inherits this one's binding constraints (artifact-only interface, no Python on the render path, vocabulary quarantine) and its measured caveats (ID collisions, `update` ignoring `--code-only`, community renumbering, confidence tags not being resolution signals) as design inputs.
>
