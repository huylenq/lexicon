...
Pages hold presentation content; their names and positions do not determine semantic membership. Combined mirrors the individual 2D planes' arrangements and authored drawings. Moving a plane in Combined changes its presentation offset. Rearranging a model item in Combined updates its owning page’s layout. New drawings belong to the active drawing plane, and edits to drawings update their owning page. Linked Sources contributes shared file and target objects, with source connections preserving the authored mappings from their model owners. Domain and Architecture navigate to these targets without expanding them inline.
...
The viewer renders references with the target's existing type or classification icon and item navigation. Hovering or focusing a reference opens a temporary Reader card without changing navigation or history. Move into the card to read it, pin it to keep it, or leave or press Escape to dismiss it. With the reference focused and its preview visible, Tab enters the card; Shift+Tab from its first control returns to the reference. List previews show readable text; opening the item exposes clickable references. An unavailable target remains readable with a dotted underline and an unavailable-item hint. References are plain-text enrichment, so existing XML storage and schema remain unchanged.
