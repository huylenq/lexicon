...
One model supplies the reader, graph filters, and derived sequence diagrams. The 2D view shows either Domain or Architecture as an exploration filter, not a complete suite of scoped C4 diagrams. Layers always shows all elements on separate Domain and Architecture planes when exploring their correspondence, without a per-layer selector. Both 2D dimensions offer Standard, Atlas Ink, and Atlas Village skins. Views and Beyond guides choosing and reviewing views; it introduces no persisted viewpoint records, framework registry, or configurable type system.
