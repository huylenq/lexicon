...
export type RepositoryFileMetric =
...
export type RepositoryInventory = {
  ...
  truncated: boolean;
  scope: "git" | "directory";
  metrics?: Record<string, RepositoryFileMetric>;
};

export function repositoryMetricLabel(metric?: RepositoryFileMetric) {
...
export const repositoryTargetId = (file: string) => `repository:${JSON.stringify(file)}`;
export function repositoryFile(id: string): string | undefined {
...
export const repositoryLink = (file: string): SourceLink => {
  const base = { file, role: "reference", description: "Repository file." };
