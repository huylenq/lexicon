...
**Combined** composes Domain, Architecture, and Linked Sources from their saved pages. Source connections link owners to the canonical Linked Sources targets using the same orthogonal routing, rounded corners, and obstacle avoidance as semantic relationships. Source links retain dashed styling; relationship-owned links depart from the relationship label. Drag a plane heading to move that plane; select its heading to make it the destination for new drawings. Drawing edits write back to the owning page, while model references retain their individual-view placements. Existing Combined layouts gain a separate Linked Sources region without resetting the other planes' offsets.
