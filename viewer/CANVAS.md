...
The toolbar uses Lexicon's shared icons and selection title; the footer shares its semantic legend. Routine save status appears in the footer. Right-click a model object for **Focus**. Cross-dimension radials connect an object or relationship to its declared sources on Linked Sources and connect source targets or file cards back to their owners. Sources are not expanded inline on Domain or Architecture. **Back to overview** restores the preceding camera.
