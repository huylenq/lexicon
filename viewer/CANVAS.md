...
Open **Edge appearance** in the canvas toolbar to adjust diagram edges. Drag the **Corner radius** preview to round bends (0–256 px); short segments and nearby labels limit the rounding automatically. **Crossing hops**, on by default, adds small upward bridges on horizontal runs and gaps in the vertical lines underneath. Rounded corners and curved source mappings also bridge crossings: the existing curve stays in place while the other edge gets a tight gap at the actual intersection. Two curves use stable edge identity to choose which passes over. Junctions, tangencies, labels, and tightly packed crossings are excluded. Rotated reference copies and Atlas roads keep their existing drawing. Both controls are browser preferences; they do not edit `canvas.json` or model XML.
...
The **2D skin** toggle group offers **Standard** model cards and rectangular context frames, **Atlas · Ink** for the pen-and-paper style, and **Atlas · Village** for illustrated terrain and buildings. Atlas is available in both the 2D Domain and Architecture views. The skin is a browser preference and does not change shared canvas appearance choices or model XML. Switching skins preserves positions, selection, and camera. Both dimensions initially use Atlas Ink, and existing Map and Atlas preferences are retained. Switching between 2D and Layers preserves the 2D skin. Concepts anchor landmarks, and visible semantic relationships supply the road routes. Panning and zooming carry the map with the canvas.
