...
function ReaderProject({ projectId }: { projectId: string }) {
  ...
  const code = (id: string, index: number) => {
    if (workspace.open) {
      selectGraph({ kind: "mapping", id: mappingId(id, index) });
      return;
    }
    const p = new URLSearchParams(params);
    p.set("code", id);
  ...
  const graphSelection: GraphSelection | undefined =
    specialSelection ||
    (params.get("code")
      ? {
          kind: "mapping",
          id: mappingId(params.get("code")!, Number(params.get("link") || 0)),
        }
      : params.get("item")
  ...
  return (
    <div
      className={`reader ${selectedLink ? "with-code" : ""} ${workspace.open ? "with-graph" : ""} ${!workspace.sidebar ? "without-sidebar" : ""} ${mobileRead ? "mobile-reading" : "mobile-graph"}`}
    >
      ...
      >
        {model && graphMounted && (
          <div className={`graph-slot ${workspace.open ? "" : "graph-closed"}`}>
            <Suspense fallback={<p className="empty">Opening graph…</p>}>
              <GraphPane
                ...
                onClearSelection={() => {
                  const p = new URLSearchParams(params);
                  for (const key of ["item", "selection", "code", "link"])
                    p.delete(key);
        ...
        <main className="reading-pane" ref={content} id="main-content">
          ...
          {graphSelection && (
            <div className="reader-graph-actions">
              <button
                className="quiet"
                onClick={() => graphAction("locate", graphSelection)}
              >
                Locate in graph
              </button>
              {item && (
