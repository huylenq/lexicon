...
Open the floating **Agent** button to discuss and refine the project's shared model. Drag the launcher to an edge or corner, drag the window header to move it, or drag any window corner to resize it. Drop the launcher onto the Canvas toolbar to dock it; drag it out to float again. The chat header also offers **Dock launcher in toolbar** and **Float launcher**. Position, size, and launcher docking survive reloads; narrow screens keep the launcher floating. Press `⌘\` (`Ctrl+\` on other platforms) to toggle the assistant. Use **Attach Agent to right side** in its header for a dedicated right pane, or **Float Agent window** to detach it. Attachment is remembered per project; narrow screens use the floating view. Hiding or moving the window preserves drafts and running conversations. A working dot stays visible on the launcher during a turn. Conversations survive navigation, reload, and server restarts. The independent bottom status bar shows the model legend and visible concept/code counts.

Choose **Codex**, **Grok**, **Claude**, **pi**, **Oh My Pi**, or **Hermes**. Install the corresponding CLI and sign in through its terminal workflow first. Lexicon checks the local runtime's authentication status; it does not ask for API keys. The model picker’s status dot shows connection health; open the picker for details and **Check connection** after signing in. Codex uses app-server, Grok and the pi-family agents speak ACP, and Claude uses streaming CLI output. Oh My Pi runs `omp acp` with auto-approval; the chat's server side still refuses write permissions those agents request. Hermes runs `hermes acp` with its configured provider credentials. pi has no native ACP server yet; the community `pi-acp` bridge (npm `pi-acp`) exposes one, and Lexicon spawns it as `pi-acp`. Lexicon creates its own sessions and resumes them on later messages.
...
Tool calls appear as they run. Expand one to inspect its input and output; completed calls remain in the conversation after reload. Long output is truncated. A call without a result is marked stopped rather than successful. Replies show elapsed time and completion state. Multiple tool calls in a reply are grouped for expansion. **Reuse prompt** puts an earlier question back in the composer for editing, and **Jump to latest** returns to the newest activity after scrolling up.

The selected concept or relationship and its source links appear above the composer. Uncheck the attachment for a project-wide question. Each sent message keeps the context captured at that moment. The circular up-arrow inside the textarea sends; **Enter** sends and **Shift+Enter** adds a line. While a reply runs, that button becomes **Stop**. When the agent asks a structured question, choose an option or type your own answer.
