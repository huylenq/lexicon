...
The **2D / Layers** switch chooses the canvas view. 2D shows Domain or Architecture on a flat canvas; Layers shows both dimensions on separate planes. The 2D domain view has a **2D skin** toggle group: Standard cards and connections, Atlas Ink, or Atlas Village. Atlas renders contexts as districts, concepts as landmarks, and relationships as paths, updating as objects move. Skins share positions and selection. Select an object in Atlas to edit its landmark, terrain, or path appearance. The skin is remembered in this browser. See [Atlas skins](CANVAS.md#atlas-skins) for the defaults and scope.
