...
The **Diagram / Atlas** mode toggle changes the canvas presentation. Diagram uses cards and connections. Atlas adds an ink village: contexts form districts, concepts become landmarks, and relationship routes become paths. It regenerates from the current canvas positions as you move objects. Both modes share positions and selection. In Atlas, select a model object to choose its landmark, terrain, or path in the appearance panel. Appearance choices are shared with the canvas; the mode is remembered in this browser. See [Atlas mode](CANVAS.md#atlas-mode) for the defaults and scope.
