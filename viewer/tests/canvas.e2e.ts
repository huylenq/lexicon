...
test("a 300-concept model opens, saves, and keeps the complete document while culling offscreen shapes", async ({ page }, info) => {
  ...
  await writeFile(join(root, "lexicon/model.xml"), `<lexicon schema="3.3" id="benchmark"><name>Canvas benchmark</name><description>300 concepts, 20 contexts, 280 relationships.</description>${contexts}${relations}</lexicon>`);
