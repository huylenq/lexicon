...
- **Anchor coverage**: count cold-layer entries missing their anchoring fields. These don't crash the loader, but they mean the cold layer has drifted toward "glossary" and away from "verifiable spec" — the failure mode lexicon is built to prevent.
  - Terms without `symbols:` and whose `definition` mentions a file, class, function, or `code-style identifier`. Report `<count>` and list the first 10 fqids; the user fills in.
  ...
  - Atoms with rationale-shaped slots empty: seams with `kind:` set but no `rationale:`, aggregates with no `rationale:`, invariants with `validationMode: principle` and no `rationale:`. Rationale-emptiness isn't a parse error — it's a thinking-debt indicator. Report `<count>` and list the first 10; the user absorbs during a future `lex-crystallize`.
  - Seams with `kind: unknown`. These load with a warning by design — the user is supposed to classify each seam, and any still-`unknown` after migration is triage.
...
- **Schema-stripped fields on system.yaml**: re-parse `lexicon/system.yaml` as raw YAML (not via the loader) and diff its top-level keys against the v0.3 canonical set: `schemaVersion, kind, id, name, purpose, narrative, body, contexts, sharedKernels, overlays, deliberateOmissions`. Anything else is invisible to the loader and should be migrated or removed. Common offenders from older schemas: `crossCuttingTerms`, `crossCuttingInvariants` (lift to `sharedKernels` via `lex-migrate v0.2→v0.3`); `inlineContexts`, `crossCuttingSeams`, `crossCuttingBoundaryRules`, `battery` (lift via `lex-migrate v0.1→v0.2` first if still present).
- **Schema-stripped fields on contexts/*.yaml**: diff each context file's top-level keys against `schemaVersion, kind, id, name, subdomain, purpose, narrative, codeModules, body, terms, invariants, seams, boundaryRules, aggregates, modules`. A `modules:` value that is `string[]` (rather than an array of objects) is a pre-v0.3 remnant — should have been renamed to `codeModules` by the migration. Flag.
