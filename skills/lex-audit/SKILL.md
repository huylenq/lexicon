...
- **Anchor coverage**: count cold-layer entries missing their anchoring fields. These don't crash the loader, but they mean the cold layer has drifted toward "glossary" and away from "verifiable spec" — the failure mode lexicon is built to prevent.
  ...
  - ADRs with empty `affects:`. Report `<count>` and list them; the affects list is what makes "what decided this?" queryable. v0.x markdown didn't carry the field, so post-`lex-migrate` projects start at 100% gap and chip away over time — surface as triage, not as a regression.
- **Schema-stripped fields on system.yaml**: re-parse `lexicon/system.yaml` as raw YAML (not via the loader) and diff its top-level keys against the canonical set: `schemaVersion, kind, id, name, purpose, body, contexts, crossCuttingTerms, crossCuttingInvariants, deliberateOmissions`. Anything else is invisible to the loader and should be removed or migrated. Common offenders from v0.x: `inlineContexts`, `crossCuttingSeams`, `crossCuttingBoundaryRules`, `battery`. Suggest distributing into the relevant context files (see `lex-migrate` Phase 6/6b/6c for the canonical destinations).
