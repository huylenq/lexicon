...
When `lex-ground` writes `docs/plans/_active/<session-id>.md`, it's a soft lock — it announces what this session is touching. Other concurrent sessions read these to detect overlap. Don't skip this even for short sessions.
...
Each session needs a unique ID for sharding files. The `lex-ground` skill mints one if `$LEXICON_SESSION_ID` isn't set, and writes it to `docs/plans/_scratch/.session-id` so subsequent skill invocations in the same session can find it. Use the same ID throughout a session.
