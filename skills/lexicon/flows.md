...
The bundle's `MODEL.md` governs Flow syntax and semantic composition; `contract.md` summarizes the authoring obligations. A Flow is a Behavior item representing one scenario. The example below is optional illustration, not an additional contract.
...
Those relationship IDs must already exist or be added in the same edit. Each supplies the endpoints. Step text names the action in this scenario; it may differ from the relationshi's broader name. The example's supporting source and annotations are in `examples/shop/lexicon/model.xml`.
...
Before authoring, state the scenario's trigger, relevant preconditions, and outcome in its description or annotations. For each step, inspect the connecting source and justify the action, direction, participants, and position in the sequence. A static association or implementation correspondence is insufficient evidence of an interaction. Explain domain participants' correspondence to executing software. Report unsupported steps and important omitted outcomes separately from structural validation results.
