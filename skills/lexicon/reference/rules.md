...
## 4. Crystallize on the user's call
...
## 5. Cold-layer edits go through `crystallize`
...
## 6. IDs are slugs; rename ≠ re-slug
...
## 7. Respect project-specific overrides in `CLAUDE.md`
...
- **Concurrent agents.** If you run multiple sessions on the same repo, lexicon doesn't coordinate them — each session reads the cold layer and does its work. Conflicts surface as ordinary git conflicts. Lexicon doesn't try to prevent this; it just stays out of the way.
