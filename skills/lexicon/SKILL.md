...
```
lexicon/
  system.yaml                       ← the cold layer root
  ...
  _pre-migrate-archive/             ← created by conform's structural pass; preserves pre-v0.3 originals
```
...
- Project has no `lexicon/system.yaml` and the user is starting substantive work → suggest `adopt` (unless the user declined recently — see "When this workflow doesn't apply" below).
...
- **`ground`** — Read `system.yaml` and the relevant context files, declare scope in conversation, surface vocabulary gaps. No file writes. The agent's context window holds the grounding.
...
- `${CLAUDE_SKILL_DIR}/reference/schema.md` — normative cold-layer YAML schema (current: v0.3). Read whenever you're emitting, mutating, or validating cold-layer YAML.
...
- `${CLAUDE_SKILL_DIR}/migrations/v0.x-to-v0.1.md`, `v0.1-to-v0.2.md`, `v0.2-to-v0.3.md` — per-version deltas used by `conform`'s structural pass. Each is self-contained: pre-flight, detection, apply phases, validate phase, report template.

YAML templates and (eventually) deterministic scripts are at:

- `${CLAUDE_SKILL_DIR}/templates/{system,bounded-context,surface}.yaml.example`, `${CLAUDE_SKILL_DIR}/templates/plan.md.template` — reference shapes used by `adopt`.
...
1. **Read `lexicon/system.yaml` first.** Before substantive work, end to end. It's small by design (~500 lines). Past that, surface for partitioning into `contexts/<slug>.yaml`. If relevant context files exist, load only the ones that match the work — don't read every context file eagerly.
...
6. **Cold-layer edits go through `crystallize`.** Don't drive-by-edit `lexicon/system.yaml`, `lexicon/contexts/*.yaml`, or `lexicon/surfaces/*.yaml` as a side effect of unrelated work. Cold-layer changes are deliberate: propose, get explicit approval, then apply. (Direct edits are fine when the user explicitly asks — "fix this typo in system.yaml.")
...
If a project's `lexicon/` declares an older `schemaVersion:` (`"0.1"` or `"0.2"`) or still has `lexicon/system.md` (legacy markdown), surface once: *"This project is on lexicon schema vX; v0.3 is current. Want to run `conform` first?"* Respect a "no"; the operational subcommands won't run cleanly until structural migration happens, and the viewer (if used) refuses to render the project.
