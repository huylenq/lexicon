...
Check semantic composition against `MODEL.md`: are domain ideas distinguished from software responsibilities, and are their correspondences explained? For every Flow, identify the trigger, relevant preconditions, and outcome. Require Architecture participants for every Flow step. Check each occurrence's action, direction, participants, and order against connecting source. For caller, callee, and callSite references, resolve the Flow-owned code-link IDs and inspect whether each target belongs to the stated responsibility and supports the action. Distinguish direct calls from HTTP, events, and internal computation; keep missing implementation unspecified. A valid reference to a static association or implementation mapping does not establish an interaction. Review important alternative outcomes within the requested scope, without treating every model as requiring a Flow or architecture inventory.
